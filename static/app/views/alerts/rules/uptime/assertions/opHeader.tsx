...
export function AssertionOpHeader({value, onChange, onRemove}: AssertionOpHeaderProps) {
  ...
  // Filter options for header value comparisons (no less_than/greater_than/always/never)
  const headerValueComparisonOptions = COMPARISON_OPTIONS.filter(
    opt => !['less_than', 'greater_than', 'always', 'never'].includes(opt.value)
  );
  ...
  const keyCombinedTooltip =
    keyOperandType === 'none'
      ...
      : t('Header key %s a string %s', keyComparisonLabel, keyOperandLabel);
  ...
  const valueCombinedTooltip =
    valueOperandType === 'none'
      ...
      : t('Header value %s a string %s', valueComparisonLabel, valueOperandLabel);

  return (
    <OpContainer label={t('Header')} onRemove={onRemove}>
      <Grid gap="sm" align="center" columns="1fr 2fr" width="100%">
        <InputGroup>
              ...
              <CompositeSelect.Region
                ...
                onChange={option => {
                  ...
                  onChange({
                    ...
                    key_operand: isAlwaysNever ? {header_op: 'none'} : value.key_operand,
                    value_op: isAlwaysNever ? {cmp: option.value} : value.value_op,
          ...
          <InputGroup>
            <InputGroup.LeadingItems>
              ...
              >
                ...
                <CompositeSelect.Region
                  label={t('String type')}
                  value={valueOperandType === 'none' ? 'literal' : valueOperandType}
                  onChange={option => {
                    const newOperand: HeaderOperand =
                      option.value === 'literal'
