...
def update_project_webhook(integration_id: int, organization_id: int, repository_id: int) -> None:
    ...
    with GitlabTaskEvent(
        ...
        try:
            ...
            logger.warning(
                "update-project-webhook.repository-not-found",
                extra={
                    "integration_id": integration_id,
                    "organization_id": organization_id,
                    "repository_id": repository_id,
            ...
            lifecycle.record_halt(GitLabWebhookUpdateHaltReason.REPOSITORY_NOT_FOUND)
            return
        ...
        if not webhook_id or not project_id:
            logger.warning(
                "update-project-webhook.missing-webhook-config",
                extra={
            ...
            lifecycle.record_halt(GitLabWebhookUpdateHaltReason.MISSING_WEBHOOK_CONFIG)
            return
...
def update_all_project_webhooks(integration_id: int, organization_id: int) -> None:
    ...
    with GitlabTaskEvent(
        ...
        lifecycle.add_extra("total_repositories", repositories.count())
