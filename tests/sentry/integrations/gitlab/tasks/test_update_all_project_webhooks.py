...
class UpdateAllProjectWebhooksTest(GitLabTestCase):
    ...
    @patch("sentry.integrations.utils.metrics.EventLifecycle.record_event")
    @patch("sentry.integrations.gitlab.tasks.update_project_webhook.delay")
    def test_task_logs_spawned_count(self, mock_delay, mock_record_event):
        """Test that the task logs the repository info"""
        with patch("sentry.integrations.gitlab.tasks.logger") as mock_logger:
            ...
            # Verify completion log with repository info
            mock_logger.info.assert_any_call(
                ...
                extra={
                    ...
                    "repository_ids": [self.repo1.id, self.repo2.id, self.repo3.id],
                },
            )
