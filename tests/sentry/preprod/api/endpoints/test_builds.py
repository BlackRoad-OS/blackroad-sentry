...
class BuildTagKeyValuesEndpointTest(APITestCase):
    ...
    def test_date_filtering(self) -> None:
        old = self.create_preprod_artifact(app_id="old", date_added=before_now(days=10))
        middle = self.create_preprod_artifact(app_id="middle", date_added=before_now(days=5))
        recent = self.create_preprod_artifact(app_id="recent", date_added=before_now(days=1))
